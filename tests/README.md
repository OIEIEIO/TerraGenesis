...
To run only MonetaVerde's core tests (after building):
...
To run only MonetaVerde's crypto tests (after building):
...
```bash
monetaverded --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `monetaverde-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only MonetaVerde's hash tests (after building):
...
To run only MonetaVerde's unit tests (after building):
